...
const App = () => {
  ...
  return (
    <>
      ...
      <div className="relative z-20 -mt-32 md:-mt-48 px-3 sm:px-4 mb-16 md:mb-32">
         <div className="max-w-7xl mx-auto">
            {/* Glassmorphic Container */}
            <div className="relative bg-gradient-to-br from-slate-900/90 via-slate-950/90 to-slate-900/90 backdrop-blur-2xl rounded-2xl md:rounded-[2.5rem] border border-white/10 shadow-2xl shadow-purple-900/30 overflow-hidden group hover:border-purple-500/30 transition-all duration-500">
               ...
               <div className="relative bg-slate-950/60 border-b border-white/10 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between backdrop-blur-md">
                  <div className="flex gap-1.5 sm:gap-2">
                     <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-red-500/30 border border-red-500/50 shadow-sm"></div>
                     <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-yellow-500/30 border border-yellow-500/50 shadow-sm"></div>
                     <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-500/30 border border-emerald-500/50 shadow-sm"></div>
                  </div>
                  <div className="h-6 sm:h-7 px-3 sm:px-6 bg-white/5 rounded-full border border-white/10 flex items-center justify-center text-[9px] sm:text-[11px] font-mono text-slate-400 backdrop-blur-sm">
                     <span className="text-purple-400 hidden sm:inline">app.</span>crewcontrol.io<span className="text-slate-500 hidden sm:inline">/dashboard</span>
                  </div>
                  <div className="w-12 sm:w-20"></div>
               </div>

               {/* Dashboard Content */}
               <div className="relative p-4 sm:p-6 md:p-8 bg-gradient-to-b from-slate-950/50 to-slate-900/30">
                  
                  {/* Top Bar - Modern Design */}
                  <div className="flex flex-col gap-4 mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-white/10">
                     {/* Left: Date Navigation */}
                     <div className="flex items-center gap-2 sm:gap-3">
                        <div className="h-9 sm:h-10 px-3 sm:px-4 bg-gradient-to-r from-purple-500/20 to-purple-600/10 rounded-lg sm:rounded-xl border border-purple-500/30 flex items-center justify-center shadow-lg shadow-purple-500/10">
                           <span className="text-xs sm:text-sm font-bold text-purple-300">Jan 2026</span>
                        </div>
                        <div className="flex gap-1.5 sm:gap-2">
                           <button className="h-8 w-8 sm:h-9 sm:w-9 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 flex items-center justify-center transition-all hover:scale-105">
                              <ChevronDown size={12} className="sm:w-[14px] sm:h-[14px] text-slate-400 rotate-90" />
                           </button>
                           <button className="h-8 w-8 sm:h-9 sm:w-9 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 flex items-center justify-center transition-all hover:scale-105">
                              <ChevronDown size={12} className="sm:w-[14px] sm:h-[14px] text-slate-400 -rotate-90" />
                           </button>
                     ...
                     <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                        {/* Stats Pills */}
                        <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 bg-purple-500/10 rounded-lg border border-purple-500/20">
                           <Users size={12} className="sm:w-[14px] sm:h-[14px] text-purple-400" />
                           <span className="text-[10px] sm:text-xs font-bold text-purple-300">24</span>
                           <span className="text-[9px] sm:text-[10px] text-slate-400">Active</span>
                        </div>
                        <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 bg-emerald-500/10 rounded-lg border border-emerald-500/20">
                           <Clock size={12} className="sm:w-[14px] sm:h-[14px] text-emerald-400" />
                           <span className="text-[10px] sm:text-xs font-bold text-emerald-300">648</span>
                           <span className="text-[9px] sm:text-[10px] text-slate-400">Hours</span>
                        </div>
                        
                        {/* View Toggle */}
                        <div className="flex gap-1 p-1 bg-white/5 rounded-lg border border-white/10 ml-auto">
                           <button className="px-2.5 sm:px-3 py-1.5 bg-purple-500/20 rounded-md border border-purple-500/30 text-[10px] sm:text-xs font-bold text-purple-300 shadow-sm">Week</button>
                           <button className="px-2.5 sm:px-3 py-1.5 text-[10px] sm:text-xs font-medium text-slate-400 hover:text-slate-300 transition-colors">Month</button>
                        </div>
                     </div>
                  </div>

                  {/* Calendar Grid - Enhanced */}
                  <div className="mb-4 overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
                     {/* Day Headers */}
                     <div className="grid grid-cols-7 gap-1.5 sm:gap-2 mb-2 sm:mb-3 min-w-[560px] sm:min-w-0">
                        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, i) => (
                           <div key={i} className="text-center text-[10px] sm:text-xs font-bold text-slate-400 py-1.5 sm:py-2 uppercase tracking-wider">
                              {day}
                           </div>
                        ))}
                     </div>
                     ...
                     <div className="grid grid-cols-7 gap-1.5 sm:gap-2 min-w-[560px] sm:min-w-0">
                        {[
                           ...
                           <div 
                              key={i} 
                              className="min-h-[100px] sm:min-h-[120px] bg-gradient-to-br from-slate-900/60 to-slate-950/60 rounded-lg sm:rounded-xl border border-white/10 p-2 sm:p-3 relative group hover:border-purple-500/40 hover:bg-slate-900/80 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10"
                           >
                              {/* Day Number */}
                              <div className="text-xs sm:text-sm font-bold text-slate-300 mb-1.5 sm:mb-2 leading-none">{date.day}</div>
                              
                              {/* Shifts */}
                              <div className="space-y-1 sm:space-y-1.5">
                                 {date.shifts.slice(0, 2).map((shift, idx) => (
                                    <div 
                                       key={idx}
                                       className={`
                                          text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-1 sm:py-1.5 rounded-md sm:rounded-lg border backdrop-blur-sm
                                    ...
                                    >
                                       {shift.type === 'emergency' ? (
                                          <span className="flex items-center gap-0.5 sm:gap-1 truncate">
                                             <span className="text-[8px] sm:text-[9px] flex-shrink-0">🚨</span>
                                             <span className="text-[8px] sm:text-[9px] truncate">Emergency</span>
                                          </span>
                                       ) : (
                                          <div className="flex flex-col gap-0.5">
                                             <span className="truncate font-semibold text-[9px] sm:text-[10px]">{shift.name.split(' ')[0]}</span>
                                             <span className="text-[8px] sm:text-[9px] opacity-80 font-medium leading-tight">{shift.startTime} - {shift.endTime}</span>
                                          </div>
                                       )}
                                    </div>
                                 ))}
                                 {date.shifts.length > 2 && (
                                    <div className="text-[9px] sm:text-[10px] text-slate-500 font-medium px-1.5 sm:px-2 py-0.5 sm:py-1 bg-white/5 rounded-md sm:rounded-lg border border-white/5">
                                       +{date.shifts.length - 2}
                                    </div>
                                 )}
                              </div>
                              ...
                              {date.shifts.length === 0 && (
                                 <div className="absolute inset-0 flex items-center justify-center opacity-20">
                                    <div className="w-6 h-6 sm:w-8 sm:h-8 border-2 border-dashed border-white/20 rounded-lg"></div>
                                 </div>
                              )}
                           </div>
                        ))}
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 pt-4 sm:pt-6 border-t border-white/10">
                     <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-500/10 rounded-lg sm:rounded-xl border border-blue-500/20">
                           <CheckCircle2 size={14} className="sm:w-4 sm:h-4 text-blue-400" />
                           <span className="text-xs sm:text-sm font-bold text-blue-300">98%</span>
                           <span className="text-[10px] sm:text-xs text-slate-400">Coverage</span>
                        </div>
                        <div className="hidden sm:block h-8 w-px bg-white/10"></div>
                        <div className="text-[10px] sm:text-xs text-slate-500">
                           <span className="font-semibold text-slate-400">648</span> hours scheduled
                        </div>
                     </div>
                     
                     <button className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2.5 bg-gradient-to-r from-purple-600 to-purple-500 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold text-white hover:from-purple-500 hover:to-purple-400 transition-all shadow-lg shadow-purple-500/20 hover:shadow-purple-500/30 hover:scale-105 flex items-center justify-center gap-2">
        ...
        <section id="features" className="py-16 sm:py-24 md:py-32 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center mb-12 sm:mb-16">
               <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-300 text-xs sm:text-sm font-semibold mb-4 sm:mb-6">
                  <Zap size={12} className="sm:w-[14px] sm:h-[14px] text-purple-400" />
                  Core Features
               </div>
               ...
               <p className="text-slate-400 max-w-2xl mx-auto text-base sm:text-lg px-4">A complete suite of tools designed to replace your spreadsheets, WhatsApp groups, and paper timesheets.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
               ...
               <div className="group relative bg-gradient-to-br from-slate-900/80 to-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 overflow-hidden hover:border-purple-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-500/20">
                  ...
                  <div className="relative z-10 mb-4 sm:mb-6">
                     <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-purple-500/20 to-purple-600/10 rounded-xl sm:rounded-2xl flex items-center justify-center border border-purple-500/30 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg shadow-purple-500/20">
                        <Calendar className="text-purple-400 w-5 h-5 sm:w-7 sm:h-7" />
                     </div>
                  </div>

                  <div className="relative z-10">
                     <h3 className="text-xl sm:text-2xl font-black mb-2 sm:mb-3 group-hover:text-purple-300 transition-colors">Smart Scheduling</h3>
                     <p className="text-slate-400 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base">Drag and drop shifts in a beautiful calendar. Check availability instantly and publish schedules with a single click.</p>
                     
                     {/* Feature Pills */}
                     <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-purple-500/10 border border-purple-500/20 rounded-full text-[10px] sm:text-xs font-semibold text-purple-300">AI Detection</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">Auto-Fill</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">One-Click Publish</span>
                     </div>

                     {/* Visual Element */}
                     <div className="relative h-24 sm:h-32 bg-slate-950/50 rounded-lg sm:rounded-xl border border-white/5 overflow-hidden group-hover:border-purple-500/30 transition-colors">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                           <div className="grid grid-cols-7 gap-0.5 sm:gap-1 w-full px-2 sm:px-4">
                              {[1, 2, 3, 4, 5, 6, 7].map((day, i) => (
                                 <div key={i} className={`
                                    h-8 sm:h-12 rounded-md sm:rounded-lg border flex items-center justify-center text-[10px] sm:text-xs font-bold
                                    ${i === 1 || i === 3 
                  ...
                  {/* Corner Accent */}
                  <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-purple-500/5 rounded-full blur-3xl -mr-12 -mt-12 sm:-mr-16 sm:-mt-16 group-hover:bg-purple-500/10 transition-colors"></div>
               </div>

               {/* Card 2: Team Management */}
               <div className="group relative bg-gradient-to-br from-slate-900/80 to-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 overflow-hidden hover:border-blue-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20">
                  {/* Animated Background Gradient */}
                  ...
                  {/* Icon with Glow */}
                  <div className="relative z-10 mb-4 sm:mb-6">
                     <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-500/20 to-blue-600/10 rounded-xl sm:rounded-2xl flex items-center justify-center border border-blue-500/30 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg shadow-blue-500/20">
                        <Users className="text-blue-400 w-5 h-5 sm:w-7 sm:h-7" />
                     </div>
                  </div>

                  <div className="relative z-10">
                     <h3 className="text-xl sm:text-2xl font-black mb-2 sm:mb-3 group-hover:text-blue-300 transition-colors">Team Management</h3>
                     <p className="text-slate-400 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base">Organize employees into teams, assign roles, and manage permissions with intuitive controls.</p>
                     
                     {/* Feature Pills */}
                     <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full text-[10px] sm:text-xs font-semibold text-blue-300">Multi-Team</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">Role-Based</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">Invites</span>
                     </div>

                     {/* Visual Element */}
                     <div className="relative h-24 sm:h-32 bg-slate-950/50 rounded-lg sm:rounded-xl border border-white/5 overflow-hidden group-hover:border-blue-500/30 transition-colors">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center p-2 sm:p-4">
                           <div className="flex gap-2 sm:gap-3 w-full">
                              <div className="flex-1 h-full bg-blue-500/20 rounded-md sm:rounded-lg border border-blue-500/30 flex flex-col items-center justify-center gap-1.5 sm:gap-2">
                                 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-blue-500/30 rounded-full"></div>
                                 <div className="h-1.5 sm:h-2 w-8 sm:w-12 bg-blue-500/20 rounded"></div>
                              </div>
                              <div className="flex-1 h-full bg-white/5 rounded-md sm:rounded-lg border border-white/5 flex flex-col items-center justify-center gap-1.5 sm:gap-2">
                                 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-white/10 rounded-full"></div>
                                 <div className="h-1.5 sm:h-2 w-8 sm:w-12 bg-white/5 rounded"></div>
                              </div>
                              <div className="flex-1 h-full bg-white/5 rounded-md sm:rounded-lg border border-white/5 flex flex-col items-center justify-center gap-1.5 sm:gap-2">
                                 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-white/10 rounded-full"></div>
                                 <div className="h-1.5 sm:h-2 w-8 sm:w-12 bg-white/5 rounded"></div>
                  ...
                  {/* Corner Accent */}
                  <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-blue-500/5 rounded-full blur-3xl -mr-12 -mt-12 sm:-mr-16 sm:-mt-16 group-hover:bg-blue-500/10 transition-colors"></div>
               </div>

               {/* Card 3: Analytics & Insights */}
               <div className="group relative bg-gradient-to-br from-slate-900/80 to-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 overflow-hidden hover:border-emerald-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-500/20">
                  {/* Animated Background Gradient */}
                  ...
                  {/* Icon with Glow */}
                  <div className="relative z-10 mb-4 sm:mb-6">
                     <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 rounded-xl sm:rounded-2xl flex items-center justify-center border border-emerald-500/30 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg shadow-emerald-500/20">
                        <BarChart3 className="text-emerald-400 w-5 h-5 sm:w-7 sm:h-7" />
                     </div>
                  </div>

                  <div className="relative z-10">
                     <h3 className="text-xl sm:text-2xl font-black mb-2 sm:mb-3 group-hover:text-emerald-300 transition-colors">Analytics & Insights</h3>
                     <p className="text-slate-400 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base">Track coverage, labor costs, and shift patterns with real-time dashboards and reports.</p>
                     
                     {/* Feature Pills */}
                     <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-[10px] sm:text-xs font-semibold text-emerald-300">Real-Time</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">Reports</span>
                        <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] sm:text-xs font-medium text-slate-300">Trends</span>
                     </div>

                     {/* Visual Element - Chart */}
                     <div className="relative h-24 sm:h-32 bg-slate-950/50 rounded-lg sm:rounded-xl border border-white/5 overflow-hidden group-hover:border-emerald-500/30 transition-colors">
                        ...
                        <div className="absolute inset-0 flex items-end justify-center gap-1 sm:gap-1.5 px-2 sm:px-4 py-2 sm:py-3">
                  ...
                  {/* Corner Accent */}
                  <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-emerald-500/5 rounded-full blur-3xl -mr-12 -mt-12 sm:-mr-16 sm:-mt-16 group-hover:bg-emerald-500/10 transition-colors"></div>
               </div>
